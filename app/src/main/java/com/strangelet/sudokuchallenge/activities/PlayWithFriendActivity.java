...
import com.strangelet.sudokuchallenge.customViews.OpponentSudokuBoard;
import com.strangelet.sudokuchallenge.customViews.SudokuBoard;
import com.strangelet.sudokuchallenge.models.Room;
...
public class PlayWithFriendActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        TextView opponentNameView = findViewById(R.id.opponent_name);
        RelativeLayout opponentNameLayout = findViewById(R.id.opponent_name_layout);
        ...
        opponentSudoku = sudokuMaker.getOpponentSudoku();
        opponentSudokuBoard.setOpponentSudoku(opponentSudoku);
        opponentSudokuBoard.setVisibility(View.INVISIBLE);
        opponentNameView.setText("Waiting for opponent...");

        if (getIntent().getStringExtra("role").equals("creator")) {
            roomReference = database.getReference().child("Rooms").child(roomCode);
            Log.d("myUser", "uploaded values");
            isCreator = true;
                ...
                public void onDataChange(@NonNull DataSnapshot snapshot) {
                    if(snapshot.exists()){
                        ...
                        if(sidekickSudoku!=null) {
                                ...
                                opponentSudokuBoard.setVisibility(View.VISIBLE);
                                if(room.getSidekick()!=null)
                                    opponentNameView.setText(room.getSidekick().getNickname() + "'s sudoku");
                        }
                    ...
                    if(snapshot.exists()){
                        ...
                        if(creatorSudoku!=null) {
                                ...
                                if(room.getCreator()!=null)
                                    opponentNameView.setText(room.getCreator().getNickname() + "'s sudoku");
                        }
        ...
        backImage.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Dialog exitDialog = new Dialog(PlayWithFriendActivity.this);
                exitDialog.setContentView(R.layout.notice_dialog);
                exitDialog.getWindow().setLayout(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT);
                exitDialog.getWindow().setBackgroundDrawable(new ColorDrawable(Color.argb(0, 0, 0, 0)));
                exitDialog.show();

                TextView confirmationQuestion = exitDialog.findViewById(R.id.confirmation_question);
                TextView confirmationYes = exitDialog.findViewById(R.id.confirmation_yes);
                TextView confirmationNo = exitDialog.findViewById(R.id.confirmation_no);

                confirmationQuestion.setText("Do you really want to exit the room? You won't be able to join it again");
    ...
    private void showFinishing(){
        ...
        PushDownAnim.setPushDownAnimTo(viewGame, returnHome).setScale(0.8f);
...
//                    .setContentType(AudioAttributes.CONTENT_TYPE_SONIFICATION) //same here as above, press ctrl+b
    ...
    protected void onPause() {
        super.onPause();
        Log.d(getIntent().getStringExtra("role"), ": deleted room");
